...
export const validateResponse = (response: any, schema: STResponse, status: number) => {
  ...
  if ((entry as STSchema)[Kind]) {
    ...
    if (response instanceof Uint8Array) s = c['application/octet-stream'] ?? c['*/*']
    else if (typeof response === 'string') s = c['text/plain'] ?? c['application/json'] ?? c['*/*']
    else if (response !== null && typeof response === 'object') s = c['application/json'] ?? c['*/*']
    else s = c['*/*']
  }
  if (!s) return
