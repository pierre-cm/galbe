...
- Returning **nothing** falls back to the default `401`: an `errorHandler` observes a rejection or replaces it, it can never authenticate one.
- Throwing takes the usual [Error Handler](../concepts/error-handler.md) path.

**`optional: true` is how optional authentication is expressed.** A request carrying **no** credential is not a rejection: it continues unauthenticated, with the state key left unset. A credential that is present and refused — or malformed, or expired — is still rejected, because a bad token is not an anonymous request.

```ts
galbe.middleware('/feed/*', jwt({ key: Bun.env.JWT_SECRET!, optional: true }))

galbe.get('/feed', ctx => (ctx.state.jwtPayload ? personalized(ctx) : generic()))
```
...
**Identity lands on `ctx.state`**, under a key named after the middleware (`jwtPayload`, `bearer`, `apiKey`, `basicAuth`), or under whatever `stateHolder` names. `verify` returns the identity to store; a **configured constant** stores `true` instead of the credential, so a secret is never copied onto the state object — under `optional`, a truthy key is still what tells an authenticated request from an anonymous one. `basicAuth` authenticates a username from its `users` map, so that name is the identity it stores.
...
| ---------------- | ---------------------------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `key`            | `string \| Uint8Array \| ArrayBuffer \| JsonWebKey \| CryptoKey` | _required_        | The verification key: the HMAC secret for `HS*`, the public key for `RS*`/`ES*`. See [Keys and algorithms](#keys-and-algorithms). |
...
| `optional`       | `boolean`                                                        | `false`           | Lets a request carrying **no** token through unauthenticated. See [Authentication](#authentication).                              |
...
`key` accepts every form a key comes in, and is imported once, lazily, on the first request:
...
```ts
jwt({ key: pem, algorithms: ['ES256'] })
```

### Sources

```ts
jwt({ key, sources: ['bearer', 'cookie:session'] })
```
...
```ts
galbe.middleware('/users/*', jwt({ key: userKey, securityScheme: 'userAuth' }))
galbe.middleware('/admin/*', jwt({ key: adminKey, securityScheme: 'adminAuth' }))
```
...
| `verify`         | `(token, ctx) => boolean \| object` |                | Looks the token up instead. Returns the identity to store; `false`/`null`/`undefined` rejects. |
| `optional`       | `boolean`                           | `false`        | Lets a request carrying **no** token through unauthenticated.                                  |
...
| `verify`         | `(key, ctx) => boolean \| object`  |                | Looks the key up instead. Returns the identity to store; `false`/`null`/`undefined` rejects. |
| `optional`       | `boolean`                          | `false`        | Lets a request carrying **no** key through unauthenticated.                                  |
...
| `verify`         | `(user, password, ctx) => boolean \| object` |                | Checks the credentials itself. Returns the identity to store; `false`/`null`/`undefined` rejects. |
| `optional`       | `boolean`                                    | `false`        | Lets a request carrying **no** credentials through unauthenticated.                               |
...
| `errorHandler` | `(info, ctx) => Response \| void` |                     | Replaces the `429`; returning nothing falls back to the default one.   |
...
All three span the whole request. They start in the [`beforeParse`](../concepts/middleware.md#before-parsing) slot — where `requestId` names the request and the other two start a clock — and `logger` and `timing` report through the [`afterHandle`](../concepts/middleware.md#after-handling) slot, which runs on the parsed `Response`, whatever the request ended on. A `400` from [validation](../concepts/schemas.md) and a `401` from an auth middleware are therefore named, logged and measured like any other answer.

What no middleware sees is a request that matched **no route** — a `404`, a CORS preflight, or a request a [Plugin](../concepts/plugins.md) answered in `onFetch`. An access log covering those belongs in the plugin.
...
| `generate`    | `() => string` | `crypto.randomUUID` | Makes an id when there is none to reuse, shaped like an inbound one. |
...
- A generated id goes through the **same shape check** an inbound one does, since it ends up in a response header and in your logs. A `generate` returning anything else fails the request with a `500` naming the function, rather than letting the `Headers` constructor reject it anonymously.
...
- **Every routed request is logged**, a `400` from validation and a `401` from an auth middleware included: `logger` starts its clock in the [`beforeParse`](../concepts/middleware.md#before-parsing) slot and writes in the [`afterHandle`](../concepts/middleware.md#after-handling) one, which runs on the parsed response of every outcome. Only a request that matched **no** route — a `404` — is out of its reach.
...
- **`duration` covers parsing and validation too**, from the pre-parse slot to the parsed response — what the server actually spent, not only what the hook chain took.
...
- `skip` runs after the request, so it can filter on the response status and keep only the failures.
...
- The measurement is taken from the [`beforeParse`](../concepts/middleware.md#before-parsing) slot to the parsed response, so a request rejected by validation or by an auth middleware carries the header as well. It measures the whole request, not the handler: the difference between the two is the parsing and validation this middleware sits around.
