...
- Middleware schema fragments: `galbe.middleware(pattern?, def)`, `group.middleware`, `galbe.group(prefix, def, cb)` and middleware files accept a `MiddlewareDef` (`{ hooks, schema, security }`). The `schema` fragment (`headers`/`query`/`params`) is merged into every matched route at registration — route-declared keys win — so validation, the OpenAPI spec, `generate client` and `generate cli` all pick up a subtree-wide request contract. `beforeParse` and `afterHandle` slot names are reserved but not run yet.
